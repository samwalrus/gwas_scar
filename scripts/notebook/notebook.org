...
#+BEGIN_SRC R :session *shell*<2> :results output :exports both
  ...
  pheno_na_pheno1 <- replace(merged_pheno1$V6.y, merged_pheno1$V6.y ==-9, 'NA')

  pheno_na_pheno2 <- replace(merged_pheno2$V6.y, merged_pheno2$V6.y ==-9, 'NA')

  pheno_na_pheno3 <- replace(merged_pheno3$V6.y, merged_pheno3$V6.y ==-9, 'NA')
...
sample_file <- read.table('/mnt/storage/private/alspacdata/freeze/original/gi_hrc_g0m_g1/da</storage/private/alspacdata/freeze/original/gi_hrc_g0m_g1/dat                               a/bgen/data.sample')

sample_file_no_head <- sample_file[-c(1,2),]

merged_pheno1 <- merge(sample_file_no_head, scar_leila_pheno1.fam, by.x ="V1", by.y="V1",al<ile_no_head, scar_leila_pheno1.fam, by.x ="V1", by.y="V1",all                               .x =TRUE)
merged_pheno2 <- merge(sample_file_no_head, scar_leila_pheno2.fam, by.x ="V1", by.y="V1",al<ile_no_head, scar_leila_pheno2.fam, by.x ="V1", by.y="V1",all                               .x =TRUE)
merged_pheno3 <- merge(sample_file_no_head, scar_leila_pheno3.fam, by.x ="V1", by.y="V1",al<ile_no_head, scar_leila_pheno3.fam, by.x ="V1", by.y="V1",all                               .x =TRUE)
...
write.table(pheno_na_pheno1, file="pheno1.txt",row.names=FALSE, col.names=FALSE,quote=FALSE<le="pheno1.txt",row.names=FALSE, col.names=FALSE,quote=FALSE)                               
write.table(pheno_na_pheno2, file="pheno2.txt",row.names=FALSE, col.names=FALSE,quote=FALSE<le="pheno2.txt",row.names=FALSE, col.names=FALSE,quote=FALSE)                               
write.table(pheno_na_pheno3, file="pheno3.txt",row.names=FALSE, col.names=FALSE,quote=FALSE<le="pheno3.txt",row.names=FALSE, col.names=FALSE,quote=FALSE)                               
#+end_example
...
<--[],
'../../gemma -g $(P)data_$(CHR).mgf -p pheno$(PhenoN).txt  -k ./output/matrix_full.sXX.txt -lmm 1 -o scar_lmm_bim_bam_chr$(CHR)_pheno$(PhenoN)'.

'./output/scar_lmm_bim_bam_chr$(CHR)_pheno$(PhenoN).log.txt' <--['./output/scar_lmm_bim_bam_chr$(CHR)_pheno$(PhenoN).assoc.txt'],{true}.

go,{bim_bam_path(P)} <--[],'../../gemma -g $(P)data_11.mgf -p pheno1.txt  -k ./output/matrix_full.sXX.txt -lmm 1 -o scar_lmm_bim_bam_chr11_pheno1'.

go2,{bim_bam_path(P)} <--[],'../../gemma -g $(P)data_11.mgf -p pheno2.txt  -k ./output/matrix_full.sXX.txt -lmm 1 -o scar_lmm_bim_bam_chr11_pheno2'.

go3,{bim_bam_path(P)} <--[],'../../gemma -g $(P)data_11.mgf -p pheno3.txt  -k ./output/matrix_full.sXX.txt -lmm 1 -o scar_lmm_bim_bam_chr11_pheno3'.

#+END_SRC

#+BEGIN_SRC bash :results output

 biomake -n './output/scar_lmm_bim_bam_chr11_pheno1.log.txt'

#+END_SRC

#+RESULTS:
:  ../../gemma -g /mnt/storage/private/alspacdata/freeze/original/omics_freeze_creation/scripts/make_gi_hrc_g0m_g1/bimbam/data_11_pheno1.mgf -p pheno.txt  -k ./output/matrix_full.sXX.txt -lmm 1 -o scar_lmm_bim_bam_chr11_pheno1
: Target ./output/scar_lmm_bim_bam_chr11_pheno1.log.txt not materialized - build required
...
#+RESULTS:
: gwscan_pheno1 <- read.table("./output/scar_lmm_bim_bam_chr11_pheno1.assoc.txt", as.is = "rs<output/scar_lmm_bim_bam_chr11_pheno1.assoc.txt", as.is = "rs"                               ,header = TRUE)
: i <- which.min(gwscan_pheno1$p_wald)
: gwscan_pheno1[i,]
:        chr          rs ps n_miss                 allele1 allele0   af
: 274158  -9 11:97798463 -9      0 11:97798463:11:97798463       G 0.02
:              beta        se   logl_H1 l_remle       p_wald
: 274158 -0.8091195 0.1753121 -1027.134   1e-05 4.620273e-06
...
#+RESULTS:
: gwscan_pheno2 <- read.table("./output/scar_lmm_bim_bam_chr11_pheno2.assoc.txt", as.is = "rs<output/scar_lmm_bim_bam_chr11_pheno2.assoc.txt", as.is = "rs"                               ,header = TRUE)
: i <- which.min(gwscan_pheno2$p_wald)
: gwscan_pheno2[i,]
:        chr           rs ps n_miss                   allele1 allele0    af
: 339914  -9 11:120136390 -9      0 11:120136390:11:120136390       T 0.049
:              beta        se   logl_H1 l_remle       p_wald
: 339914 -0.6410735 0.1467823 -808.2652   1e-05 1.486097e-05
...
#+RESULTS:
: gwscan_pheno3 <- read.table("./output/scar_lmm_bim_bam_chr11_pheno3.assoc.txt", as.is = "rs<output/scar_lmm_bim_bam_chr11_pheno3.assoc.txt", as.is = "rs"                               ,header = TRUE)
: i <- which.min(gwscan_pheno3$p_wald)
: gwscan_pheno3[i,]
:        chr           rs ps n_miss                   allele1 allele0    af
: 349911  -9 11:123700024 -9      0 11:123700024:11:123700024       C 0.091
:             beta         se   logl_H1   l_remle       p_wald
: 349911 0.4244439 0.09715521 -802.1332 0.0107884 1.478568e-05
...
#+begin_example
  ...
  dataframe <- read_dta('/user/work/sn0550/gwas_scar/scripts/hit_snp/bcg_pheno_ids<er/work/sn0550/gwas_scar/scripts/hit_snp/bcg_pheno_ids_geno_merge.dta')
