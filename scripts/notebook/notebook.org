...
#+BEGIN_SRC R :session *shell*<2> :results output :exports both
 ...
 %%% This was wrong
  row <- which(gwscan_chr11$ps==27451931) 
  gwscan_chr11[row,]
...
#+BEGIN_SRC R :session *shell*<2> :results output :exports both
 ...
 row <- which(gwscan_pheno3$ps==27451931) 
  gwscan_pheno3[row,]
...
#+BEGIN_SRC R :session *shell*<2> :results output :exports both
 ...
 row <- which(hrc_15_gwscan_chr11$rs=='11:27451931_G_A') 
...
Look up the snp we wanted to find
#+BEGIN_SRC R :session *shell*<2> :results output :exports both

 %row <- which(snptest_pheno1_output$rsid=='11:46548754') 
%% This was wrong
 row <- which(snptest_pheno1_output$rsid=='11:27451931') 
...
Lets try again building the .sample file from Leilas file.

#+BEGIN_SRC R :session *shell*<2> :results output :exports both 
  library(haven)
  leila_pheno  <- read_dta('/user/work/sn0550/gwas_scar/scripts/hit_snp/bcg_pheno_ids_geno_merge.dta')

  leila_pheno$alnqlet <- paste(leila_pheno$aln,"M",sep="")

hrc15sample_file_o

   new_join <- left_join(hrc15sample_file_o,leila_pheno, by=c("V1"="alnqlet"))

 new_join <- new_join[ -c(8,9,13,14,15,16,17,18)]

new_join[1,8] <- 'pheno_scar3'
new_join[1,9] <- 'pheno_scar1'
new_join[1,10] <- 'pheno_scar2'
...
write.table(new_join, file="pheno_all.sample",row.names=FALSE, col.names=FALSE,quote=FALSE)
#+END_SRC
...
snptest1_new,{bgen_path(P)} <--[],'snptest -missing_code NA,-9 -pheno pheno_scar1 -data $(P)/data_11.bgen pheno_all.sample -o pheno1_new.out -method score -frequentist 1 '. 
snptest2_new,{bgen_path(P)} <--[],'snptest -missing_code NA,-9 -pheno pheno_scar2 -data $(P)/data_11.bgen pheno_all.sample -o pheno2_new.out -method score -frequentist 1 '. 
snptest3_new,{bgen_path(P)} <--[],'snptest -missing_code NA,-9 -pheno pheno_scar3 -data $(P)/data_11.bgen pheno_all.sample -o pheno3_new.out -method score -frequentist 1 '. 
#+END_SRC


Read output Look up the snp we wanted to find
#+BEGIN_SRC R :session *shell*<2> :results output :exports both

   library(tidyverse)

   snptest_pheno1_output2 <- read.table("pheno1_new.out", header = TRUE)
   i2 <- which.min(snptest_pheno1_output2$frequentist_add_pvalue)
   snptest_pheno1_output2[i2,]


  row2 <- which(snptest_pheno1_output2$rsid=='11:27451931') 
  snptest_pheno1_output2[row2,]
...
snptest_pheno1_output2[row2,]
       alternate_ids        rsid chromosome position alleleA alleleB  index
421878   11:27451931 11:27451931         11 27451931       A       G 421878
       average_maximum_posterior_call     info cohort_1_AA cohort_1_AB
421878                        0.99698 0.819041     737.364     10.6337
       cohort_1_BB cohort_1_NULL  all_AA  all_AB     all_BB    all_NULL
421878  0.00100708   0.000976562 737.364 10.6337 0.00100708 0.000976562
       all_total    all_maf missing_data_proportion frequentist_add_pvalue
421878       748 0.00710946             6.52782e-07            8.54174e-11
       frequentist_add_info frequentist_add_beta_1 frequentist_add_se_1 comment
421878              0.60154               -2.58259             0.397888    <NA>
#+end_example



Lets look at the one snp by extracting from bgenn and coverting to ped

#+BEGIN_SRC prolog :tangle makeprog 

'subset_11.gen',{bgen_path(P)} <--[],'qctool -g $(P)/data_11.bgen -og subset_11.gen -incl-rsids  id.txt'.

'plink_binary' <--['subset_11.gen'],'qctool -g subset_11.gen -filetype gen -og one_snp -ofiletype binary_ped -s pheno_all.sample'.

'plink_ped' <--['plink_binary'],'plink --bfile one_snp --recode --tab --out one_snp_ped'.
#+END_SRC

#+BEGIN_SRC R :session *shell*<2> :results output :exports both  
   library(haven)
  leila_pheno_read  <- read_dta('/user/work/sn0550/gwas_scar/scripts/hit_snp/bcg_pheno_ids_geno_merge.dta')
  ped_read <- read.table('one_snp_ped.ped')

  test_sample <- which(leila_pheno_read$aln=='30919') 
  leila_pheno_read[test_sample,]

  test_sample <- which(leila_pheno_read$aln=='51422') 
  leila_pheno_read[test_sample,]
#+END_SRC
