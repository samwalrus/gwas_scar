...
** Aim compute the p-values p-values to assess support for association between scar length and SNPs on chromosomes 1–22.
...
['scar.log.txt','scar.assoc.txt'] <--['pheno.csv',,BGens,SampleFile],
'./gemma 
\-p pheno.txt -c covar.txt -a map.txt -g geno.txt -notsnp -lm 2 -outdir . -o tibia'.

[gemma_basic] <--['scar.log.txt','scar.assoc.txt'].
#+END_SRC
