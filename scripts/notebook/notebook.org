...
: library(RNOmni)
: 
: pheno_with_scar$scar_size_rank_transform <-RankNorm(pheno_with_scar$fm4bc023<rank_transform <-RankNorm(pheno_with_scar$fm4bc023)                          
: 
: summary(pheno_with_scar$scar_size_rank_transform)
:      Min.   1st Qu.    Median      Mean   3rd Qu.      Max. 
...
* 
* 
* TODO The fam file that is written has 10 extra rows compared to the combined.fam file that is written in. So we need to fix this.
* TODO This is because of the A and B qlets which have the same scar value.
#+BEGIN_SRC R :session *shell*<2> :results output :exports both
  ...
  dim(subset(pheno_with_scar,duplicated(alnqlet_m)))

  subset(pheno_with_scar,duplicated(alnqlet_m))

  ### list of dupes
  dupes <- pheno_with_scar %>% group_by(alnqlet_m) %>% tally %>% filter(n > 1) %>% select(1)

  ### select rows where there is a duplicated m value
  pheno_with_scar[pheno_with_scar$alnqlet_m %in% dupes$alnqlet_m,]



  %>% select(Price)
  pheno_with_scar %>% group_by(alnqlet_m) %>% ta

  x[x %in% unique(x[duplicated(x)])]


  ### We can see that the 13 duplicates are because of qlet b.
  library(dplyr)
