...
*** TODO todo check the blink file format of hrc is best guess or dossage?
...
We create the standardized relatedness matrix using Gemma option:
“-gk 2” 
...
'test.txt',{hrc_plink_path(P)}<--[],'echo $(P) > test.txt'.

'./output/scar$(CHR).assoc.txt',{hrc_plink_path(P)} <--['pheno.csv','covar.csv', 'geno_$(CHR).txt'],
'./gemma 
\-p pheno.csv -c covar.csv -a map.txt -g geno_$(CHR).txt -notsnp -lm 2 -outdir . -o scar_$(CHR).

'./output/scar_$(CHR).log.txt' <--['./output/scar$(CHR).assoc.txt'],{true}.

[gemma_basic_$(CHR)] <--['./output/scar_$(CHR).log.txt'].
#+END_SRC

We can evoke a dry run of the recipes to check the commands are correct.
