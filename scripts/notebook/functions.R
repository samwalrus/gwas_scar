...
plot.inflation <- function (x, size = 2) {
  ...
  # to smallest.
  y <- rev(sort(-log10(x),na.last =FALSE))
...
plot.gwscan <- function (gwscan, size = 1) {
  ...
  gwscan <- transform(gwscan,p_wald = -log10(p_wald))
  ...
  return(ggplot(gwscan,aes(x = marker,y = p_wald,color = odd.chr)) +
         geom_point(size = size,shape = 20) +
         scale_x_continuous(breaks = x.chr,labels = 1:23) +
...
plot.region.pvalues <- function (gwscan, size = 1) {
  ...
  return(ggplot(gwscan,aes(x = ps,y = p_wald)) +
